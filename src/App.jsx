...
const CompressPDFTool = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (level === 'high') {
        const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        // Bolt: Chunked parallel rendering to speed up compression while avoiding OOM on large PDFs
        const chunkSize = 4;
        for (let i = 1; i <= pdf.numPages; i += chunkSize) {
          ...
          const renderedPages = await Promise.all(chunk.map(async (pageNum) => {
          ...
          // Process embedding sequentially to preserve exact page order and prevent internal pdfDoc issues
          for (const renderData of renderedPages) {
            const embeddedImg = await pdfDoc.embedJpg(renderData.imgBytes);
            const newPage = pdfDoc.addPage([renderData.width, renderData.height]);
            newPage.drawImage(embeddedImg, { x: 0, y: 0, width: renderData.width, height: renderData.height });
          }
        }
      } else {
