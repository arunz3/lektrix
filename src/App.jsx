...
const CompressPDFTool = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (level === 'high') {
        const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        // Bolt: Chunked parallelization for faster compression while avoiding OOM on large PDFs
        const CHUNK_SIZE = 3;
        for (let i = 1; i <= pdf.numPages; i += CHUNK_SIZE) {
          ...
          const processedChunk = await Promise.all(chunk.map(async (pageNum) => {
            ...
            return { embeddedImg, viewport };
          }));

          // Process sequentially to preserve order
          for (const { embeddedImg, viewport } of processedChunk) {
