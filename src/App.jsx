...
const FileUpload = ({
  ...
  const handleFiles = (newFiles) => {
    let processableFiles = Array.from(newFiles);

    if (multiple) {
      const remainingSlots = maxFiles !== undefined ? Math.max(0, maxFiles - files.length) : processableFiles.length;
      processableFiles = processableFiles.slice(0, remainingSlots);
    } else {
      processableFiles = processableFiles.slice(0, 1);
    }

    const fileList = processableFiles.map(file => ({
