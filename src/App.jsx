...
const FileUpload = ({
  ...
  const handleFiles = (newFiles) => {
    let rawFilesArray = Array.from(newFiles);

    if (multiple) {
        // Enforce maxFiles limit before processing
        const remainingSpace = maxFiles !== undefined ? Math.max(0, maxFiles - files.length) : undefined;
        if (remainingSpace !== undefined) {
            rawFilesArray = rawFilesArray.slice(0, remainingSpace);
        }
    } else {
        rawFilesArray = rawFilesArray.slice(0, 1);
    }

    const fileList = rawFilesArray.map(file => ({
            ...
            {files.length > 0 && (
              <div className="space-y-6 pt-4 border-t border-gray-100">
                <div>
                  ...
                  <input type="text" placeholder="e.g. 1, 3, 5-8" value={pagesInput} onChange={(e) => setPagesInput(e.target.value)} maxLength={500} className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent transition-all" />
            ...
            {files.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 gap-4 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-xl border border-gray-100 dark:border-slate-800">
                ...
                <div className="space-y-1">
                  ...
                  <input type="text" value={metadata.subject} onChange={e => setMetadata({...metadata, subject: e.target.value})} maxLength={255} className="w-full bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-accent outline-none transition-all" placeholder="e.g. Financial Data" />
                </div>
                <div className="space-y-1">
                  ...
                  <input type="text" value={metadata.keywords} onChange={e => setMetadata({...metadata, keywords: e.target.value})} maxLength={500} className="w-full bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-accent outline-none transition-all" placeholder="e.g. report, stats, 2025" />
                </div>
              </motion.div>
            )}
                ...
                <div className="space-y-4">
                  ...
                  {mode === 'protect' && (
                    <div>
                      ...
                      <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} maxLength={128} className="w-full px-4 py-3.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl outline-none focus:border-accent transition-all dark:text-white" />
                    </div>
                  )}
                </div>
                {error && <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl text-sm font-medium border border-red-100 dark:border-red-900/30 flex items-center gap-2"><AlertCircle size={18} /> {error}</div>}
          ...
          <div className="space-y-8">
            ...
            {files.length > 0 && (
              <div className="space-y-6 pt-4 border-t border-gray-100 dark:border-slate-800">
                {type === 'text' ? (
                  <div className="space-y-4">
                    <input type="text" value={wmText} onChange={(e) => setWmText(e.target.value)} maxLength={255} className="w-full px-4 py-3.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl outline-none focus:border-accent dark:text-white" placeholder="Watermark text" />
