...
const CompressPDFTool = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (level === 'high') {
        const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        // Bolt Optimization: Process pages in parallel chunks to reduce total compression time.
        // Impact: Significantly speeds up high-level compression for multi-page PDFs
        // while preventing Out-of-Memory (OOM) errors by limiting concurrency to 3 pages at a time.
        const chunkSize = 3;
        for (let i = 1; i <= pdf.numPages; i += chunkSize) {
          ...
          const processedChunk = await Promise.all(chunk.map(async (pageNum) => {
            ...
            return { embeddedImg, width: viewport.width, height: viewport.height };
          }));

          for (const { embeddedImg, width, height } of processedChunk) {
