  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (level === 'high') {
        ...
        // Bolt: Process in batches of 3 to avoid OOM while still parallelizing
        const BATCH_SIZE = 3;
        for (let i = 1; i <= pdf.numPages; i += BATCH_SIZE) {
          ...
          const processedPages = await Promise.all(batch.map(async (pageNum) => {
          ...
          for (const { imgBytes, width, height } of processedPages) {
