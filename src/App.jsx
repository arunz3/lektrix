  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (level === 'high') {
        ...
        // Bolt: Chunked parallel rendering to speed up compression without OOM
        const chunkSize = 3;
        for (let i = 1; i <= pdf.numPages; i += chunkSize) {
          ...
          const renderedChunk = await Promise.all(chunk.map(async (pageNum) => {
          ...
          for (const { imgBytes, width, height } of renderedChunk) {
