...
const ImageToPDFTool = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // Parallelize file reading and image embedding
      const imageTasks = files.map(async (imgData) => {
        const bytes = await imgData.file.arrayBuffer();
        const img = imgData.file.type === 'image/jpeg'
          ? await pdfDoc.embedJpg(bytes)
          : await pdfDoc.embedPng(bytes);
        return img;
      });

      const embeddedImages = await Promise.all(imageTasks);

      // Process the resulting embedded images sequentially to preserve order
